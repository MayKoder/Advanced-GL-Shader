...
void MaterialImporter::CreateBaseMaterialFile(const char* path)
{
	...
	root_object.WriteString("ShaderUID", EngineExternal->moduleScene->defaultMaterial->shader->GetUID().c_str());
...
void MaterialImporter::Save(ResourceMaterial* material, char** fileBuffer)
{
	...
	root_object.WriteString("ShaderUID", material->shader->GetUID().c_str());
